...
class MoveIndicator(Indicator):
    ...
    def search_actions(self):
        ...
        self.game.add_object(IndicatorObject(self.game, self.entity, self.entity.x, self.entity.y, self.entity.z - 1, self.indicator, self))
    ...
    def _check_and_append_position(self, dx, dy, dz):
        ...
        if block and block.solid and (not block_above or not block_above.solid) and not self.game.get_entity(x, y, z + 1):
            self.game.add_object(IndicatorObject(self.game, self.entity, x, y, z, self.indicator, self))
